...
TEST(library, Cpu_Memory_Copy) {
  ...
  aml::cpu::memory::copy(a.data(), b.data(), a.size());
...
TEST(library, Cpu_Memory_Swap) {
  ...
  aml::cpu::memory::swap(a.data(), b.data(), a.size());
...
TEST(library, Cpu_Tensor_Scale) {
  ...
  aml::cpu::tensor::scale(a.data(), 2.0f, a.size());
...
TEST(library, Cpu_Tensor_Update) {
  ...
  aml::cpu::tensor::update(a.data(), b.data(), 1.0f, a.size());
...
TEST(library, Cpu_Tensor_Fixedupdate) {
  ...
  aml::cpu::tensor::fixed_update(a.data(), 1.0f, a.size());
...
TEST(library, Cpu_Tensor_Sum) {
  std::array a = {0.0f, 1.0f, 2.0f, 3.0f, 4.0f};
  EXPECT_EQ(aml::cpu::tensor::sum(a.data(), a.size()), 10.0f);

  std::array b = {-1.0f, 0.0f, 1.0f};
  EXPECT_EQ(aml::cpu::tensor::sum(b.data(), b.size()), 0.0f);
}

TEST(library, Cpu_Tensor_Mean) {
  std::array a = {0.0f, 1.0f, 2.0f, 3.0f, 4.0f};
  EXPECT_EQ(aml::cpu::tensor::mean(a.data(), a.size()), 2.0f);

  std::array b = {-1.0f, 0.0f, 1.0f};
  EXPECT_EQ(aml::cpu::tensor::mean(b.data(), b.size()), 0.0f);
}
...
TEST(library, Cpu_Tensor_Min) {
  ...
  EXPECT_EQ(aml::cpu::tensor::min(a.data(), a.size()), 0.0f);
  EXPECT_EQ(aml::cpu::tensor::min(b.data(), b.size()), -1.0f);
}
...
TEST(library, Cpu_Tensor_Max) {
  ...
  EXPECT_EQ(aml::cpu::tensor::max(a.data(), a.size()), 4.0f);
  EXPECT_EQ(aml::cpu::tensor::max(b.data(), b.size()), 1.0f);
}
...
TEST(library, Cpu_Tensor_Normalize) {
  ...
  aml::cpu::tensor::normalize(a.data(), &mean, &std, a.size());
...
TEST(library, Cpu_Tensor_Denormalize) {
  ...
  aml::cpu::tensor::denormalize(a.data(), mean, std, a.size());
...
TEST(library, Cpu_Linalg_Dot) {
  ...
  EXPECT_EQ(aml::cpu::linalg::dot(a.data(), b.data(), a.size()), 35.f);
}
...
TEST(library, Cpu_Linalg_Matmul) {
  ...
  */
  ...
  aml::cpu::linalg::matmul(a.data(), b.data(), c.data(), 2, 2, 3);
...
TEST(library, Cpu_Nn_Relu) {
  ...
  aml::cpu::nn::relu(a.data(), a.size());
  EXPECT_EQ(a, relu_a);
}

TEST(library, Cpu_Stats_Stddeviation) {
  std::array a = {1.0f, 2.0f, 3.0f, 4.0f, 5.0f, 6.0f, 7.0f};
  EXPECT_EQ(aml::cpu::stats::std_deviation(a.data(), a.size()), 2.0f);
}

TEST(library, Cpu_Stats_Variance) {
  std::array a = {0.0f, 1.0f, 2.0f, 3.0f, 4.0f};
  EXPECT_EQ(aml::cpu::stats::variance(a.data(), a.size()), 2.0f);

  std::array b = {0.11f, 0.62f, 0.53f, 0.41f, 0.93f, 0.02f, 0.74f};
  EXPECT_EQ(aml::cpu::stats::variance(b.data(), b.size(), true), 0.1076f);
}
...
TEST(library, Cpu_Stats_Covariance) {
  ...
  EXPECT_EQ(aml::cpu::stats::covariance(a.data(), b.data(), a.size()), 4.6f);
  EXPECT_EQ(aml::cpu::stats::covariance(a.data(), b.data(), a.size(), true), 5.75);
}

// TEST(library, Cpu_Random_Uniform) {
//   std::array<float, 1000> a;
//   aml::cpu::random::uniform(a.data(), a.size(), 0.f, 1.f);
